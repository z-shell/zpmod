...
int zp_source_study_core(const char *nam, int report_count, int threshold_ms, int clear_history) { // NOLINT(bugprone-easily-swappable-parameters)
  (void)threshold_ms; (void)clear_history; (void)nam;
  if (report_count < 0) { return 1; }
  return 0; /* no-op */
}
...
static int (*zp_source_study_core_anchor)(const char *, int, int, int) __attribute__((used)) = zp_source_study_core;
#endif
