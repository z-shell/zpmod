 ...
 *
 * Generates function-to-path mappings from FPATH directories with performance optimization
 * through skip detection that avoids rebuilding unchanged indexes.
 *
 * ## Index Format (v1)
 *
 ...
 */
int cmd_fpath_index(char *nam, char **argv) { // NOLINT(readability-function-cognitive-complexity)
    const char *outfile = NULL;
    int rebuild = 0;
    int preload = 0;

    while (*argv && argv[0][0] == '-') {
        if (strcmp(argv[0], "--rebuild") == 0) { rebuild = 1; argv++; continue; }
        if (strcmp(argv[0], "--preload") == 0) { preload = 1; argv++; continue; }
        if (strcmp(argv[0], "--out") == 0) {
            if (!argv[1]) { zwarnnam(nam, "--out requires an argument"); return 1; }
            outfile = argv[1]; argv += 2; continue;
        }
     ...
     *
     * Attempts to detect when the existing index is still fresh and can be reused,
     * avoiding expensive directory scanning when FPATH directories haven't changed.
     *
    ...
    if (outfile && !rebuild) {
        ...
        if (rf) {
            ...
            if (fgets(line, sizeof(line), rf) && strncmp(line, "# zpmod fpath-index v1", 22) == 0) {
                /*
                 * HEADER VALIDATION STRATEGY
                 *
                 * We require the header to completely cover the current fpath configuration.
                 * This ensures that any changes to fpath ordering, additions, or removals
                 * will trigger a rebuild as expected by test scenarios.
                 *
                ...
                while (fgets(line, sizeof(line), rf)) {
                    ...
                    if (sscanf(line, "# %7s %d %2047s %ld", tag, &i_hdr, pathbuf, &mt_recorded) != 4) {
                        header_ok = 0;
                        break;
                    }
                     ...
                     */
                    struct stat st_now;
                    long mt_current = -1; /* Default for missing/inaccessible directories */

                    if (stat(fpath[idx], &st_now) == 0) {
                        mt_current = (long)st_now.st_mtime;
                    }
                    /* If stat() fails, mt_current remains -1, matching generation behavior */
    ...
    if (outfile) {
        ...
        if (!out_fp) { zwarnnam(nam, "cannot open for writing: %s", outfile); return 1; }
    ...
    if (outfile) {
        ...
        for (int i = 0; fpath[i]; ++i) {
            long mt = -1;
            if (stat(fpath[i], &st) == 0) { mt = (long)st.st_mtime; }
            fprintf(out_fp, "# dir %d %s %ld\n", i, fpath[i], mt);
        }
    }

    for (int i = 0; fpath[i]; i++) {
        DIR *d = opendir(fpath[i]); if (!d) { continue; }
        struct dirent *de; while ((de = readdir(d))) {
            ...
            if (!preload) { fprintf(out_fp, "%s %s/%s\n", de->d_name, fpath[i], de->d_name); }
        }
        closedir(d);
    }

    if (outfile && out_fp != stdout) { fclose(out_fp); }
    return 0;
}
