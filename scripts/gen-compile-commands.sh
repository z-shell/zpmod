...
# shellcheck shell=sh
# shellcheck disable=SC2292  # Keep POSIX [ ] tests for /bin/sh portability
...
# Configure if needed (POSIX test)
...
if cmake --build "${BUILD_DIR}" --target copy-compile-commands >/dev/null 2>&1; then :; else
  cmake --build "${BUILD_DIR}" -j 2 >/dev/null || cmake --build "${BUILD_DIR}" >/dev/null
fi

# As a final step, ensure the root copy exists for tools like clangd and Trunk
