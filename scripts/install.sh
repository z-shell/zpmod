...
if ! printf '%s\n%s\n' "${ZSH_REQUIRED}" "${ZSH_CURRENT}" | sort -V -C; then
  ...
  (
    ...
    # CMake configure/build directories
    BUILD_DIR="${INSTALL_DIR}/build-cmake"
    OUT_LIB="${BUILD_DIR}/out/lib"

    # Clean or reconfigure as requested
    if [ "${CLEAN_BUILD}" -eq 1 ] && [ -d "${BUILD_DIR}" ]; then
      info "${col_info2}-- Removing build directory for a clean build --${col_rst}"
      rm -rf "${BUILD_DIR}"
    fi

    # Configure with CMake and custom CFLAGS
    verbose "-- CMake configure (CFLAGS: ${CUSTOM_CFLAGS}) --"
    cmake -S . -B "${BUILD_DIR}" \
      -DCMAKE_BUILD_TYPE=Release \
      -DCMAKE_C_FLAGS="${CUSTOM_CFLAGS}" || {
      error "CMake configure failed."
      exit 255
    }

    # Determine number of jobs for build
    if [ -z "${JOBS}" ]; then
    ...
    info "${col_info2}-- Building with ${cores} jobs --${col_rst}"

    # Build with CMake, capture output based on verbosity
    if [ "${VERBOSE_MODE}" -eq 1 ]; then
      ...
      if ! cmake --build "${BUILD_DIR}" --parallel "${cores}" >build.log 2>&1; then
        error "Module didn't build. See build.log for details."
        exit 255
      fi
    fi
    ...
    if [ -z "${ARTIFACT}" ]; then
      error "Module artifact not found under ${OUT_LIB}"
      ls -la "${OUT_LIB}" 2>/dev/null || true
      exit 255
    fi
